...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        public_password = os.environ.get("PUBLIC_DEMO_PASSWORD", "PublicPass123!")

        org_acme = Organization.objects.create(
            name="Acme Corporation", slug="acme", plan="enterprise",
            settings_json={"features": ["sso", "audit", "webhooks"], "max_users": 50},
        )
        ...
        admin_user = User.objects.create_superuser(
            ...
            password=os.environ.get("SYSADMIN_PASSWORD", secure_random_token(24)), role="sys_admin",
            organization=org_acme, first_name="System", last_name="Admin",
        )
        admin_user.preferences = {"theme": "dark"}
        admin_user.save(update_fields=["preferences"])

        acme_admin = User.objects.create_user(
            username="acme_admin", email="admin@acme.local",
            password=os.environ.get("ACME_ADMIN_PASSWORD", secure_random_token(24)), role="org_admin",
        ...
        globex_admin = User.objects.create_user(
            ...
            password=os.environ.get("GLOBEX_ADMIN_PASSWORD", secure_random_token(24)), role="org_admin",
        ...
        agent1 = User.objects.create_user(
            ...
            password=os.environ.get("ACME_AGENT_PASSWORD", secure_random_token(24)), role="agent",
            organization=org_acme, first_name="Carol", last_name="Support", department="IT Support",
        )
        agent2 = User.objects.create_user(
            username="agent_dave", email="dave@globex.local",
            password=os.environ.get("GLOBEX_AGENT_PASSWORD", secure_random_token(24)), role="agent",
            organization=org_globex, first_name="Dave", last_name="Tech", department="Engineering",
        )

        user1 = User.objects.create_user(
            username="user_eve", email="eve@acme.local",
            password=os.environ.get("ACME_USER_PASSWORD", secure_random_token(24)), role="user",
            organization=org_acme, first_name="Eve", last_name="Employee", department="Marketing",
        )
        user2 = User.objects.create_user(
            username="user_frank", email="frank@globex.local",
            password=os.environ.get("GLOBEX_USER_PASSWORD", secure_random_token(24)), role="user",
            organization=org_globex, first_name="Frank", last_name="Staff", department="Sales",
        )

        public_user = User.objects.create_user(
            username="public_user", email="public@vulnops.local", password=public_password, role="user",
            organization=org_public, first_name="Public", last_name="User", department="General", bio="",
        )

        APIToken.objects.create(user=public_user, name="Public API Access", token=os.environ.get("PUBLIC_API_TOKEN", secure_random_token(64)), is_active=True)

        svc_account = User.objects.create_user(
            username="svc_integration", email="svc@vulnops.local",
            password=os.environ.get("SERVICE_ACCOUNT_PASSWORD", secure_random_token(24)), role="service_account",
            organization=org_acme, first_name="Service", last_name="Bot", is_internal=True,
        )
        APIToken.objects.create(user=svc_account, name="Integration Token", token=os.environ.get("SERVICE_API_TOKEN", secure_random_token(64)), is_active=True)
        APIToken.objects.create(user=admin_user, name="Admin Token", token=os.environ.get("SYSADMIN_API_TOKEN", secure_random_token(64)), is_active=True)

        now = timezone.now()
        tickets_data = [
            {"title": "VPN disconnects intermittently", "description": "Users in Building A report VPN disconnects every 30 minutes. Affects approximately 50 users. Started after the network upgrade last Friday.", "status": "open", "priority": "high", "category": "network", "reporter": user1, "assignee": agent1, "org": org_acme},
            ...
            {"title": "Server room temperature alert", "description": "Environmental monitoring shows server room B2 at 28C, threshold is 25C. HVAC maintenance required urgently.", "status": "escalated", "priority": "critical", "category": "infrastructure", "reporter": agent1, "assignee": acme_admin, "org": org_acme, "is_internal": True, "internal_notes": "HVAC vendor contract redacted."},
        ...
        for i, td in enumerate(tickets_data):
            t = Ticket.objects.create(
                ...
                is_internal=td.get("is_internal", False), internal_notes=td.get("internal_notes", ""),
        ...
        for i, td in enumerate([
            ...
            Ticket.objects.create(
                organization=td["org"], title=td["title"], description=td["description"],
                status=td["status"], priority=td["priority"], reporter=td["reporter"], assignee=td.get("assignee"),
        ...
        Comment.objects.create(ticket=created_tickets[0], author=agent1, content="I've checked the VPN concentrator logs. Seeing timeout errors from the new firmware. Rolling back to previous version.", content_html="<p>I've checked the VPN concentrator logs. Seeing timeout errors from the new firmware. Rolling back to previous version.</p>")
        Comment.objects.create(ticket=created_tickets[0], author=user1, content="Thanks Carol. Some users are still experiencing issues after the rollback. Can you check again?", content_html="<p>Thanks Carol. Some users are still experiencing issues after the rollback. Can you check again?</p>")
        Comment.objects.create(ticket=created_tickets[1], author=agent1, content="Permissions issue confirmed. The service account password expired. Resetting now.", content_html="<p>Permissions issue confirmed. The service account password expired. Resetting now.</p>")
        Comment.objects.create(ticket=pub_ticket, author=public_user, content="Adding a comment to the public test ticket.", content_html="<p>Adding a comment to the public test ticket.</p>")

        Incident.objects.create(organization=org_acme, title="Major network outage - Building A", description="Complete network loss in Building A affecting 200+ users. Switch failure in MDF confirmed.", severity="critical", status="mitigating", reporter=agent1, commander=acme_admin, affected_services=["email", "vpn", "file-shares", "voip"], detected_at=now - timedelta(hours=6))
        Incident.objects.create(organization=org_globex, title="Email delivery delays", description="Outbound emails delayed by 2-4 hours. Mail queue backing up on relay server.", severity="medium", status="investigating", reporter=agent2, commander=globex_admin, affected_services=["email"], detected_at=now - timedelta(hours=2))
        ...
        for ad in assets_data:
            Asset.objects.create(organization=ad["org"], name=ad["name"], asset_tag=ad["tag"], asset_type=ad["type"], serial_number=ad["sn"], manufacturer=ad["mfg"], model=ad["model"], location=ad["loc"], department=ad["dept"], purchase_cost=ad["cost"], status="active")

        WebhookConfig.objects.create(organization=org_acme, name="Slack Notifications", url=os.environ.get("WEBHOOK_RECEIVER_URL", "http://mock-webhook-receiver:9002") + "/webhook/slack", secret=os.environ.get("WEBHOOK_SIGNING_SECRET", secure_random_token(32)), events=["ticket.created", "ticket.updated", "incident.created"], created_by=acme_admin, is_active=True)
        WebhookConfig.objects.create(organization=org_globex, name="PagerDuty Integration", url=os.environ.get("WEBHOOK_RECEIVER_URL", "http://mock-webhook-receiver:9002") + "/webhook/pagerduty", secret=os.environ.get("WEBHOOK_SIGNING_SECRET", secure_random_token(32)), events=["incident.created", "ticket.escalated"], created_by=globex_admin, is_active=True)

        AuditEvent.objects.create(user=admin_user, organization=org_acme, action="login", resource_type="session", description="System admin logged in", ip_address="10.0.0.1")
