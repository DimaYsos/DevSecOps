...
function AuthProvider({ children }) {
  ...
  useEffect(() => {
    auth.me().then(r => setUser(r.data)).catch(() => setUser(null)).finally(() => setLoading(false));
  }, []);
    ...
    <>
      ...
      <div className="page-content">
        <div className="card" style={{ maxWidth: 600 }}>
          ...
          <p style={{ marginBottom: 16, color: 'var(--gray-500)', fontSize: 13 }}>Upload a file to import assets. Supported formats: CSV, JSON, YAML, PKL.</p>
          <div className="form-group">
            <input type="file" onChange={e => setFile(e.target.files[0])} accept=".csv,.json,.yaml,.yml,.pkl,.pickle" />
          </div>
...
function PasswordResetPage() {
  ...
  const requestReset = async (e) => {
    ...
    try {
      ...
      setMsg(`Token sent! Debug token: ${r.data.debug_token || 'check email'}`);
      setStep(2);
